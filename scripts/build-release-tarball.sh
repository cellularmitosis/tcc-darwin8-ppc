...
# producing tcc-darwin8-ppc-v0.1.0-g3.tar.gz in the repo root.
...
# Step 2: bootstrap tcc-self (= gcc-built tcc compiles tcc.c).
echo "==> step 2/4: bootstrap tcc-self"
WORK=/tmp/$PKGNAME-build/self1
OUT=/tmp/$PKGNAME-build/tcc-self
mkdir -p "$WORK"
TCC=./tcc/tcc OUT="$OUT" WORK="$WORK" ./scripts/bootstrap-tcc-self.sh
...
First self-hosting release. Targets 32-bit PowerPC on Mac OS X 10.4
Tiger (Mach-O). Built and bootstrapped on a real iMac G3.
...
See $PREFIX/share/doc/tcc-darwin8-ppc/SELF_HOST_NOTES for the
self-host milestone writeup.
